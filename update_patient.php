...
if (isset($_POST['new']) && $_POST['new'] == 1) {
    ...
    $update = "UPDATE patient SET profile_photo = :profile_photo, first_name = :first_name, last_name = :last_name, gender = :gender, dob = :dob, blood_group = :blood_group, phone = :phone, state = :state, city = :city, address = :address, aadhar = :aadhar, email = :email WHERE id = :id ";
    ...
    if ($result) {
        ...
        echo '<script>window.location.href = "view_patient_profile.php?id=' . $id . '";</script>';
    } else {
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <!-- Add this script after the City dropdown -->
    <script>
        document.getElementById('inputState').addEventListener('change', function () {
            var state = this.value;
            var cityDropdown = document.getElementById('inputDistrict');
            cityDropdown.innerHTML = '';

            var cities = [];
            switch (state) {
                case 'Gujarat':
                    cities = ["Ahmedabad", "Baroda", "Surat", "Rajkot", "Bhavnagar"];
                    break;
